...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) => 
      option.setName("emoji")
        .setDescription("検索するデフォルト絵文字(1文字)")
        .setRequired(true)
    )
    .addNumberOption((option) =>
      option.setName("threshold")
        .setDescription("対象にするリアクション数")
        .setRequired(true)
    ),
  ...
  async handleComponents(interaction){
    ...
    const okMessages = allMessages.filter((message) => 
      message.reactions.cache.get(emoji)?.count >= threshold
    ).reverse();
